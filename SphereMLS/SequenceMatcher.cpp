...
void SequenceMatcher::processTwoImageOverCommonMask(const cv::Mat &image1, const cv::Mat &image2,
								   const cv::Mat &mask1, const cv::Mat &mask2,
								   PairInfo &pairInfo, int index1 = 0, int index2 = 1)
{
	...
	cv::Mat commonMask = mask1 & mask2;
	m_featurator->detectAndCompute(image1, commonMask, keypoints1, descriptor1);
	m_featurator->detectAndCompute(image2, commonMask, keypoints2, descriptor2);

	std::vector<cv::DMatch> matches;
	_getCorresponds(descriptor1, descriptor2, matches, 0.7);
	pairInfo.pairs_num = matches.size();
	pairInfo.inliers_num = matches.size();
	pairInfo.mask.resize(matches.size(), 1);
	pairInfo.index1 = index1;
	pairInfo.index2 = index2;
	pairInfo.points1.resize(matches.size());
	pairInfo.points2.resize(matches.size());
	for (size_t i = 0; i < matches.size(); i++)
	{
		pairInfo.points1[i] = keypoints1[matches[i].queryIdx].pt;
		pairInfo.points2[i] = keypoints2[matches[i].trainIdx].pt;
	}
}
...
void DrawPairInfos(std::vector<cv::Mat> &images, std::list<PairInfo> &pairinfos, std::string name, bool onlyPoints, double scale)
{
...
void ShowPairInfos(std::vector<cv::Mat> &images, std::list<PairInfo> &pairinfos, double scale)
{
	...
	for (std::list<PairInfo>::iterator iter = pairinfos.begin(); iter != pairinfos.end(); iter++)
	{
		...
		std::vector<cv::Scalar> vColor(iter->pairs_num);
		for (size_t i = 0; i < iter->pairs_num; i++)
		{
			if (iter->mask[i] != 1)continue;
			uchar r = rand() % 255;
			...
			cv::Scalar color(b, g, r);
			vColor[i] = color;
			cv::circle(result, iter->points1[i] * scale, 2, color, -1);
			cv::Point2d pt2 = iter->points2[i] * scale;
			pt2.x += left.cols;
			//if (!onlyPoints)cv::line(result, iter->points1[i] * scale, pt2, color, 3);
			cv::circle(result, pt2, 2, color, -1);
			//cv::imshow("showtemp", result);
			//cv::waitKey(0);
		}

		ss << index;
		std::string winname = "pair_" + ss.str();
		ss.str("");
		cv::Mat lineDraw;
		...
		{
			if (iter->mask[i] != 1)continue;
			result.copyTo(lineDraw);
			cv::Scalar color = vColor[i];
			cv::Point2d pt2 = iter->points2[i] * scale;
			pt2.x += left.cols;
			cv::line(lineDraw, iter->points1[i] * scale, pt2, color, 1);

			cv::imshow(winname, lineDraw);
			cv::waitKey(0);
		}
